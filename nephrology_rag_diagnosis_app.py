...
혈액검사 수치를 기반으로 진단을 보조하고,
질병에 관한 질문에 근거 기반 응답을 제공하는 **AI 기반 진단 지원 도구**입니다.

**기능 소개:**
1. 수치 직접 입력 또는 검사 이미지 업로드(OCR 기반)
2. 룰 기반 진단 및 리포트 저장
3. RAG 기반 질의응답 (유사 문서 및 AI 설명)

아래에서 원하는 기능을 선택해 주세요 👇
""")

st.header("🧪 검사 수치 입력 또는 이미지 인식")
...
# OCR 또는 직접입력 결과 수치 기반으로 질문 생성 후 RAG 실행
def run_rag_from_input(eGFR, creatinine, albumin, proteinuria):
    user_question = f"eGFR: {eGFR}, Creatinine: {creatinine}, Albumin: {albumin}, 단백뇨: {proteinuria} 이 수치로 어떤 질병이 의심되나요?"
...
st.subheader("📷 이미지 업로드 후 OCR 인식")
uploaded_file = st.file_uploader("PDF 또는 이미지 파일 업로드", type=["pdf", "png", "jpg", "jpeg"])
...
if uploaded_file:
    ...
    prot_val = extract_value("단백뇨|Proteinuria")

    st.write("**📌 추출된 검사 수치:**")
    st.write(f"eGFR: {eGFR_val}, Creatinine: {creat_val}, Albumin: {alb_val}, 단백뇨: {prot_val}")
