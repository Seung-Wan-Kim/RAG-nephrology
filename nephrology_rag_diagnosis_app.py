...
def run_rag_from_input(eGFR, creatinine, albumin, proteinuria):
    ...
    st.download_button("📥 수치 기반 리포트 다운로드", data=report_buffer.getvalue(), file_name="질병예측_리포트.docx")
...
if user_question:
    ...
    st.download_button("📥 질의응답 리포트 다운로드", data=report_buffer.getvalue(), file_name="질의응답_리포트.docx")
